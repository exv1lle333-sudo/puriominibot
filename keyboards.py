...
from config import (
    ...
    PRIVACY_POLICY_URL, TERMS_OF_USE_URL, WEBAPP_URL,
)
...
def main_menu_kb(user_id: int) -> InlineKeyboardMarkup:
    ...
    # Кнопка мини-приложения (Purio App) появляется автоматически, как только
    # в .env указан WEBAPP_URL (см. config.py и webapp/README.md).
    if WEBAPP_URL:
        kb.button(text="🐉 Открыть Purio App", web_app=WebAppInfo(url=WEBAPP_URL))
        rows.append(1)
